...
parse_args() {
  ...
  [[ -n "$DOMAIN" ]] || { error "--domain is required"; exit 1; }
  [[ -n "$DB_USER" ]] || { error "--db-user is required"; exit 1; }
  [[ -n "$DB_PASSWORD" ]] || { error "--db-password is required"; exit 1; }
  [[ -n "$JWT_SECRET" ]] || { error "--jwt-secret is required"; exit 1; }
}
