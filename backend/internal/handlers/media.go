...
func UploadMedia(c *gin.Context) {
    ...
    if cfg.IsDevelopment() {
        url = fmt.Sprintf("http://localhost:%s/api/upload/media/%s/%s/%s/%s", cfg.Server.Port, mediaKind, time.Now().Format("2006"), time.Now().Format("01"), time.Now().Format("02"))
        // 上面按天生成了目录，但为了简化，直接使用 relPath 更可靠
        url = fmt.Sprintf("http://localhost:%s/api/upload/media/%s/%s", cfg.Server.Port, mediaKind, strings.TrimPrefix(relPath, "/uploads/media/"))
    } else {
